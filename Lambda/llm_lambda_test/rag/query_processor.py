...
class QueryProcessor:
    ...
    def rewrite_query(
        ...
        Falls back to the original string if generation fails or looks invalid.
        """
        start = time.time()
        base = query or ""
        try:
            ...
            rewrite_prompt = (
                "Optimize this user query for retrieval.\n"
                + history_section
                + "Preserve meaning, optionally add clear domain keywords, under 50 tokens. "
                "Output only the rewritten query.\n"
            )

            # Attempt LLM-based rewrite if loader provided
            rewritten_raw = base
            if self.model_loader:
                try:
                    gen = self.model_loader.generate(rewrite_prompt, max_tokens=80)
                    rewritten_raw = gen.strip()
                except Exception as e:  # pragma: no cover
                    logger.warning(f"ModelLoader rewrite failed ({e}); using original")
    ...
    def embed_query(
        ...
        try:
            ...
            if self.model_loader:
                try:
                    embedding = self.model_loader.embed(embed_input)
                except Exception as e:  # pragma: no cover
                    logger.error(f"ModelLoader embedding failed ({e}); falling back to get_embeddings")
                    embedding = get_embeddings(embed_input)
            else:
                embedding = get_embeddings(embed_input)
